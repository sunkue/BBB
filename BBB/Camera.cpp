...
void Camera::update(float time_elpased)
{
	...
	/* target */
	{
		auto moving_speed = reinterpret_cast<DynamicObj*>(_ownner)->get_speed();
		auto moving_len = glm::length(moving_speed);
		auto moving_dir = glm::normalize(moving_speed);
		auto head_dir = _ownner->get_head_dir();
		if (moving_len < 0.125f)
		{
			moving_dir = head_dir;
		}
		auto target_dir = glm::lerp(_ownner->get_head_dir(), moving_dir, clamp(0.2f * moving_len, 0.00f, 0.4f));
	...
	if (_shaking)
	{
		...
		_position.x += cos(_shaking_time * friq * rand_val) * magic_num;
		_position.y += sin(_shaking_time * friq * rand_val) * magic_num;
		_position.z += cos(_shaking_time * friq * rand_val) * magic_num;
	}
